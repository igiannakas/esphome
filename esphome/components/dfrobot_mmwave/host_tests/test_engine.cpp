...
TEST(sen0395_range_quantised_no_false_mismatch) {
  ...
  CHECK(s.idle(60000));
  CHECK(s.radar.count("setRange 0.45 6") == 1);
...
TEST(uart_output_write_always_on_change) {
  for (Model m : {Model::MODEL_SEN0395, Model::MODEL_SEN0609}) {
    Sim s(m);
    CHECK(s.booted());
    CHECK(s.radar.uart_mode == 0);  // factory: periodic
...
TEST(c4001_max_range_floor) {
  ...
  CHECK_NEAR(param_limits(Model::MODEL_SEN0395, P::PARAM_ID_RANGE_MAX).lo, 0, 1e-6);
  Sim s(Model::MODEL_SEN0609);
  CHECK(s.booted());
